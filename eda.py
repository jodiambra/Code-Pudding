...
for n in number:
    ...
    px.bar(top_artist, title='Top ' + str(n) +
           ' Most Common Artists of the Decade', height=600).show()
...
for n in number:
    ...
    px.bar(top_artist, title='Top ' + str(n) +
           ' Most Common Artists of the Decade as Percentage', height=600, log_y=True).show()
...
for n in number:
    for category in categories:
        ...
        px.bar(top_artist, title='Top ' + str(n) + ' Most Common ' +
               str(category) + ' of the Decade', height=600).show()

# Top Spotify listeners
px.bar(full.user_id.value_counts().nlargest(20),
       title='Top Spotify Listeners', height=800).show()
...
px.pie(data_frame=top_listener_artists, values=top_listener_artists['count'],
       ...
       title='Favorite 10 Artists of Top Listener', height=700).show()
...
for i in year:
    ...
    px.histogram(yr, y='pop', x='artist',
                 title='Most Popular Artists of ' + str(i), height=800).show()
...
for n in number:
    ...
    px.bar(top_artist, title='Top ' + str(n) +
           ' Artists of the Decade', height=600).show()
...
for n in number:
    ...
    px.bar(top_genre, title='Top ' + str(n) +
           ' Genres of the Decade', height=600).show()
...
for i in category:
    for n in num:
        ...
        px.bar(top_category, title='Top ' + str(n) + ' ' +
               str(i) + ' of the Decade', height=600).show()
...
px.bar(unpopular.explicit.value_counts(), color=[
       'Explicit', 'Non Explicit'], title='Distribution of Explicit Songs Amongst Unpopular Tracks')

# category for tracks in unpopular songs
category = unpopular.loc[:, 'danceability':'duration_ms'].columns
for i in category:
    px.histogram(unpopular[i], title='Distribution of ' + str.upper(i) +
                 ' for Tracks Amongst Unpopular Songs', height=600).show()
